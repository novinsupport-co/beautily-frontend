...
import { baseInput, baseSubmit } from './formkit.palette'

export function rootClasses(section: string, node: FormKitNode) {
  const type = node.props.type

  const map: Record<string, Record<string, string>> = {
    text: {
      input: baseInput,
      message: 'text-red-600 font-iransans-bold text-sm mt-1',
    },
    email: {
      input: baseInput,
      message: 'text-red-600 font-iransans-bold text-sm mt-1',
    ...
    checkbox: {
      decorator: 'border border-gray-400 rounded text-fuchsia-600 focus:ring-fuchsia-500',
      input: 'hidden',
      label: 'ml-2 font-iransans-bold text-gray-700 dark:text-white',
      message: 'text-red-600 font-iransans-bold text-sm mt-1',
    },
    submit: {
      outer: 'w-full',
